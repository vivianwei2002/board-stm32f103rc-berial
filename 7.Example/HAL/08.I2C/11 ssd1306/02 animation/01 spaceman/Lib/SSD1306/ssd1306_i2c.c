...
void ShowSpaceman()
{
    ...
    SSD1306_ShowIMG(95, 4, 32, 4, SPACEMAN[frame]);
    if (++frame > 39) frame = 0;
}
void ShowTempLogo(uint8_t x, uint8_t y) { SSD1306_ShowIMG(x, y, 25, 4, TEMP); }
void ShowHumidityLogo(uint8_t x, uint8_t y) { SSD1306_ShowIMG(x, y, 19, 4, WATER); }
void ShowDegreeLogo(uint8_t x, uint8_t y) { SSD1306_ShowIMG(x, y, 3, 1, DEGREE); }
